...
class ReplayBuffer:
    ...
    Implements random sampling for breaking correlations between sequential experiences.
    """
...
class QLearningController:
    ...
    Action space: Combined discrete values for steer, acceleration, drift, brake, and nitro.
    State: Discretized aim_point and velocity.
    Reward:
      - small constant penalty each step
      - bonus/penalty based on whether last action and aim_point[0] share sign
      - penalty for low velocity (crash)
      - penalty for rescue
    ...
    def __init__(self, alpha=0.1, gamma=0.9999, epsilon=0.1,
                 ...
                 use_replay=True, replay_capacity=10000, batch_size=32):
        self.alpha = alpha
        self.gamma = gamma
        self.epsilon = epsilon
        
        # Expanded action space
        self.steer_values = [-1.0, -0.5, 0, 0.5, 1.0]
        #self.steer_values = [-1.0, 0, 1.0]
        self.accel_values = [1.0]
        self.drift_values = [False]
        self.brake_values = [False]
        self.nitro_values = [False]
        
        # Generate all possible action combinations
        self.actions = []
        for steer in self.steer_values:
            for accel in self.accel_values:
                for drift in self.drift_values:
                    for brake in self.brake_values:
                        for nitro in self.nitro_values:
                            self.actions.append((steer, accel, drift, brake, nitro))
    ...
    def getState(self, aim_point, current_vel, progress):
        ...
        current_vel: float (0 to ~10)
        progress: float between 0 and 1 indicating progress on track (unused)
        
        Returns a tuple (aim_bucket, vel_bucket)
        """
        # Discretize aim_point.x into 5 buckets
        aim_x = aim_point[0]
        if aim_x <= -0.5:
            aim_bucket = -2
        elif aim_x < 0:
        ...
        # Discretize velocity into 3 buckets
        if current_vel < 1.5:  # crashed or very slow
            vel_bucket = 0
        elif current_vel < 10.0:  # medium speed
            vel_bucket = 1
        else:  # high speed
            vel_bucket = 2
            
        # No longer using progress_bucket
        return (aim_bucket, vel_bucket)
        

    def add_finish_reward(self):
        # Use a less aggressive discount to ensure the finish reward stays significant
        # Base reward + time-dependent component
        steps_taken = len(self.cum_reward)
        base_reward = 0.7 * self.finish_reward  # 70% of reward is guaranteed
        time_bonus = 0.3 * self.finish_reward * (self.gamma ** steps_taken)  # 30% is time-dependent
        ...
        terminal_state = -2
        if self.use_replay:
            # Add terminal experience to replay buffer with done=True
            self.replay_buffer.add(self.prev_state, self.prev_action, reward, terminal_state, True)
            self.learn_from_replay()
        else:
            self.update(self.prev_state, self.prev_action, terminal_state, reward)

    def __call__(self, aim_point, current_vel, progress, t=0):
        ...
        if self.prev_state is not None:
            ...
            # 2) directional reward based on steering direction and aim_point
            aim_bucket = state[0]
            steer_value = action[0]
            
            # Reward based on how well the steering matches the aim_bucket
            if aim_bucket == -2:  # far left
                if steer_value == -1.0: reward += 2.0
                elif steer_value == -0.5: reward += 1.0
                elif steer_value == 0: reward -= 0.5
                elif steer_value == 0.5: reward -= 1.0
                elif steer_value == 1.0: reward -= 2.0
            elif aim_bucket == -1:  # moderate left
                if steer_value == -1.0: reward += 1.0
                elif steer_value == -0.5: reward += 2.0
                elif steer_value == 0: reward += 0.0
                elif steer_value == 0.5: reward -= 1.0
                elif steer_value == 1.0: reward -= 2.0
            elif aim_bucket == 0:  # center
                if steer_value == -1.0: reward -= 2.0
                elif steer_value == -0.5: reward -= 1.0
                elif steer_value == 0: reward += 2.0
                elif steer_value == 0.5: reward -= 1.0
                elif steer_value == 1.0: reward -= 2.0
            elif aim_bucket == 1:  # moderate right
                if steer_value == -1.0: reward -= 2.0
                elif steer_value == -0.5: reward -= 1.0
                elif steer_value == 0: reward += 0.0
                elif steer_value == 0.5: reward += 2.0
                elif steer_value == 1.0: reward += 1.0
            elif aim_bucket == 2:  # far right
                if steer_value == -1.0: reward -= 2.0
                elif steer_value == -0.5: reward -= 1.0
                elif steer_value == 0: reward -= 0.5
                elif steer_value == 0.5: reward += 1.0
            ...
            # 3) Heavily penalize low velocity (likely crashed)
            if current_vel < 1.0:
                reward -= 10.0
                
            # 4) Penalize rescue (if triggered this step)
            if t - self.last_rescue_time == 1 and t > 1:
                reward -= 20.0
                self.rescue_count += 1
                self.last_rescue_time = t
            ...
            if self.epsilon > 0:  # only update in training
                if self.use_replay:
                    # Add experience to replay buffer with done=False
                    self.replay_buffer.add(self.prev_state, self.prev_action, reward, state, False)
        ...
        # Extract individual action components
        steer, accel, drift, brake, nitro = action
    ...
    def learn_from_replay(self):
        ...
        for state, action, reward, next_state, done in batch:
            ...
            if done or next_state == -2:
                future_q = 0.0
            else:
                future_q = max(self.Q[(next_state, a)] for a in self.actions)
        ...
        if os.path.isfile(fn):
            ...
            with open(fn, mode='r', newline='') as f:
                ...
                for row in reader:
                    ...
                    action = eval(row[1])  # restore tuple
                    q_value = float(row[2])
...
class PyTux:
    ...
    def rollout(self, track, controller, planner=None,
                max_frames=5000, verbose=False, data_callback=None):
        ...
        for t in range(max_frames):
            ...
            action = controller(aim_image, current_vel, progress, t)

            if current_vel < 1.0 and t - last_rescue > RESCUE_TIMEOUT:
                last_rescue = t
                controller.last_rescue_time = t  # Update controller's rescue tracking
                action.rescue = True
...
if __name__ == '__main__':
    ...
    parser.add_argument('--no-replay', dest='use_replay', action='store_false',
                        help='Disable experience replay (use traditional Q-learning)')
    parser.add_argument('--replay-capacity', type=int, default=10000,
                        help='Size of replay buffer')
    parser.add_argument('--batch-size', type=int, default=32,
                        help='Batch size for replay learning')
# verbose defaults to True
    ...
    for track in args.track:
        print(f'=== TRACK: {track} ===')
        if args.use_replay:
            print(f'  Using experience replay: buffer_size={args.replay_capacity}, batch_size={args.batch_size}')
        else:
            print('  Using traditional Q-learning (no replay)')
